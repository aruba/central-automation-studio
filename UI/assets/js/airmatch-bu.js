...
function getAPRFNeighbours(radioMac, band) {
	$.when(getRFNeighboursForRadio(radioMac, band)).then(function() {
	...
	$.each(optSearchRadios, function() {
		...
		for (var i = 0;i<searchAPs.length; i++) {
			...
			if (foundAP && foundAP.name === currentAP.name) {
				// Found matching... save the radio for faster loading next time
				if (!usedProcessedData) vrfOptimizationAPs.push(optRadio)
		...
		processCounter++;
		if (optimizationNotification) optimizationNotification.update({ progress: processCounter/optSearchRadios.length*100 });		
	});
