	...
	$.ajax(settings).done(function(response) {
		if (response.hasOwnProperty('message')) {
			...
			$.each(response.vlans, function() {
				...
				//console.log(this);
				vlanTable.row.add([this['id'], this['name'], this['ipaddress'], this['tagged_ports'].join(','), this['untagged_ports'].join(',')]);
			});
			$('#vlan-table')
				.DataTable()
				.rows()
...
function findVariablesForVlan(vlanID) {
	...
	$('#vlan-table')
		.dataTable()
		.fnUpdate(temp, currentVLANRow, undefined, false);
}
...
function loadSwitchTable() {
	...
	$.each(allSwitches, function() {
		...
		if (templateSwitch) {
			...
			templateTable.row.add(['<strong>' + device['name'] + '</strong>', status, device['ip_address'], device['model'], device['serial'], device['firmware_version'], device['site'], device['group_name'], device['macaddr']]);
		}
	});
	$('#template-switch-table')
		.DataTable()
